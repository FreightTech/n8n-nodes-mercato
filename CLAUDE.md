...
This package provides specialized n8n nodes for OpenMercato integration. It includes a REST API node (auto-generated from OpenAPI spec), plus NATS messaging nodes for sending commands, consuming events, and publishing events.
...
│   └── OpenMercatoRestApi/         # REST API node (auto-generated from OpenAPI)
│       ├── OpenMercatoRestApi.node.ts
│       └── properties.ts           # Generated by @algolia/n8n-openapi-node
├── utils/
...
│   └── generate-openapi-node.ts    # Generate REST API node from OpenAPI spec
...
```bash
# Regenerate REST API node from OpenAPI spec
npm run generate:openapi

# Regenerate with fresh spec download
...
The `OpenMercatoApi` credentials (NATS) require:
...
The `OpenMercatoRestApi` credentials (HTTP) require:

- **Base URL** (e.g., `https://openmercato.freighttech.org`)
...
- OpenMercato REST API node (auto-generated from OpenAPI spec)
- OpenMercato REST API credentials (Bearer token)
